...
// define function for Easy difficulty CPU move
...
// define function for Medium difficulty CPU move (blocks player or chooses randomly)
void mediumComputerMove() {
    int row, col;

    // First, try to block the player if they are about to win
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (board[i][j] == ' ') {
                board[i][j] = computer; // Try the computer's move
                if (isWinner(computer)) { // Check if this move wins
                    return; // If the move wins, make the move and return
                }
                board[i][j] = player; // Try the player's move
                if (isWinner(player)) { // Check if this move blocks the player from winning
                    board[i][j] = computer; // Block the player's move
                    return;
                }
                board[i][j] = ' '; // Reset the spot if it doesn't block or win
            }
        }
    }
...
// define function for Hard difficulty CPU move (Advanced strategy could go here, but for simplicity, it’s random now)
void hardComputerMove() {
    // For now, the medium difficulty is the same as the hard difficulty, but you can use minimax to consider all possible moves and have the model calculate the outcome of each move.
    mediumComputerMove();
}
...
void initializeGame() {
    ...
    while (difficulty < 1 || difficulty > 3) {
        cout << "Invalid choice. Please choose a valid difficulty (1 - Easy, 2 - Medium, 3 - Hard): ";
        cin >> difficulty;
    }
}
...
int main() {
    ...
    while (true) { // loop will run until game ends - every move, this while loop is checked.
        ...
        if (difficulty == 1) {
            easyComputerMove(); // Easy move
        } else if (difficulty == 2) {
            mediumComputerMove(); // Medium move
        } else {
            hardComputerMove(); // Hard move, but for this example it is equivalent to medium still, unless minimax is implemented.
        }
